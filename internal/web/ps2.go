...
func (s *Server) ps2FPKGEnqueue(w http.ResponseWriter, r *http.Request) {
	var request struct {
		GameIDs []string `json:"game_ids"`
	}
	if err := decodeJSON(r, &request); err != nil {
	...
	items, err := s.app.PS2.EnqueueFPKG(request.GameIDs)
	if err != nil {
		writeError(w, http.StatusBadRequest, err)
		return
	}
...
func (s *Server) ps2FPKGQueueItem(w http.ResponseWriter, r *http.Request) {
	item, ok := s.app.PS2.FPKG.Get(r.PathValue("id"))
	if !ok {
		writeError(w, http.StatusNotFound, fmt.Errorf("PS2 FPKG job not found"))
		return
	}
	writeJSON(w, http.StatusOK, item)
}

func (s *Server) ps2FPKGCancel(w http.ResponseWriter, r *http.Request) {
	if err := s.app.PS2.FPKG.Cancel(r.PathValue("id")); err != nil {
		writeError(w, http.StatusConflict, err)
		return
	}
	w.WriteHeader(http.StatusNoContent)
...
func (s *Server) ps2FPKGRetry(w http.ResponseWriter, r *http.Request) {
	if err := s.app.PS2.FPKG.Retry(r.PathValue("id")); err != nil {
		writeError(w, http.StatusConflict, err)
		return
	}
	w.WriteHeader(http.StatusNoContent)
