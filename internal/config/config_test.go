...
func TestLoadDefaultsAndEnvironment(t *testing.T) {
	for _, name := range []string{"PS3MGR_GAME_DIR", "PS3MGR_REMOTE_GAME_DIR", "PS3MGR_LISTEN", "PS3_FTP_USER", "PS3_FTP_PASSWORD", "PS3MGR_WORKERS", "PS3MGR_FTP_TIMEOUT"} {
		t.Setenv(name, "")
	}
	cfg, err := Load()
	if err != nil {
	...
	if cfg.GameDir != "." || cfg.Listen != "127.0.0.1:8080" || cfg.RemoteGameDir != "/dev_hdd0/GAMES" {
	...
	if cfg.GameDir != "/games" || cfg.Workers != 12 || cfg.FTPTimeout != 1500*time.Millisecond {
